/*!
 * \file cuda_multicorrelator_real_codes_test.cc
...
DEFINE_int32(cuda_multicorrelator_real_codes_iterations_test, 100, "Number of averaged iterations in CUDA multicorrelator test timing test");
DEFINE_int32(cuda_multicorrelator_real_codes_max_threads_test, 12, "Number of maximum concurrent correlators in CUDA multicorrelator test timing test");

void run_correlator_cuda_real_codes(cuda_multicorrelator_real_codes* correlator,
...
{
    for (int k = 0; k < FLAGS_cuda_multicorrelator_real_codes_iterations_test; k++)
    {
...
TEST(CUDAMulticorrelatorRealCodesTest, MeasureExecutionTime)
{
    std::chrono::time_point<std::chrono::system_clock> start, end;
    std::chrono::duration<double> elapsed_seconds(0);
    int max_threads = FLAGS_cuda_multicorrelator_real_codes_max_threads_test;
    std::vector<std::thread> thread_pool;
    cuda_multicorrelator_real_codes* correlator_pool[max_threads];
    ...
    in_cuda = static_cast<gr_complex*>(volk_gnsssdr_malloc(2 * d_vector_length * sizeof(gr_complex), volk_gnsssdr_get_alignment()));
    ...
    {
        in_cuda[n] = std::complex<float>(uniform_dist(e1), uniform_dist(e1));
    }

    int device = init_cuda();
    if(device == -1) {
        exit(0);
    }
    cuComplex *cu_in;
    ...
    EXPECT_NO_THROW(
            for (int correlation_sizes_idx = 0; correlation_sizes_idx < 3; correlation_sizes_idx++) {
                for (int current_max_threads = 1; current_max_threads < (max_threads + 1); current_max_threads++)
                {
                    ...
                    {
                        thread_pool.push_back(std::thread(run_correlator_cuda_real_codes,
                    ...
                    execution_times[correlation_sizes_idx] = elapsed_seconds.count() / static_cast<double>(FLAGS_cuda_multicorrelator_real_codes_iterations_test);
                    std::cout << "CUDA Multicorrelator (real codes) execution time for length=" << correlation_sizes[correlation_sizes_idx]
                              << " : " << execution_times[correlation_sizes_idx] << " [s]" << std::endl;
                    printf("%f+%fi  %f+%fi  %f+%fi\n",
                           d_correlator_outs[0].real(), d_correlator_outs[0].imag(),
                           d_correlator_outs[1].real(), d_correlator_outs[1].imag(),
                           d_correlator_outs[2].real(), d_correlator_outs[2].imag());
                }
            });
