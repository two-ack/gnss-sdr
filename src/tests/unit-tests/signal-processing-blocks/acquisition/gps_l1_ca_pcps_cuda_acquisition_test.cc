 ...
 * GpsL1CaPcpsCUDAAcquisition class.
...
class GpsL1CaPcpsCUDAAcquisitionTest_msg_rx;

typedef boost::shared_ptr <GpsL1CaPcpsCUDAAcquisitionTest_msg_rx> GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_sptr;

GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_sptr
GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_make(concurrent_queue<int> &queue);


class GpsL1CaPcpsCUDAAcquisitionTest_msg_rx : public gr::block {
private:
    friend GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_sptr
    GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_make(concurrent_queue<int> &queue);

    void msg_handler_events(pmt::pmt_t msg);

    GpsL1CaPcpsCUDAAcquisitionTest_msg_rx(concurrent_queue<int> &queue);
...
public:
    ...
    ~GpsL1CaPcpsCUDAAcquisitionTest_msg_rx();  //!< Default destructor
};


GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_sptr
GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_make(concurrent_queue<int> &queue) {
    return GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_sptr(
            new GpsL1CaPcpsCUDAAcquisitionTest_msg_rx(queue));
}


void GpsL1CaPcpsCUDAAcquisitionTest_msg_rx::msg_handler_events(pmt::pmt_t msg) {
    try {
...
GpsL1CaPcpsCUDAAcquisitionTest_msg_rx::GpsL1CaPcpsCUDAAcquisitionTest_msg_rx(
        concurrent_queue<int> &queue) : gr::block("GpsL1CaPcpsCUDAAcquisitionTest_msg_rx",
                                                  gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
                                        channel_internal_queue(queue) {
    this->message_port_register_in(pmt::mp("events"));
    this->set_msg_handler(pmt::mp("events"),
                          boost::bind(&GpsL1CaPcpsCUDAAcquisitionTest_msg_rx::msg_handler_events, this, _1));
    rx_message = 0;
}
...
class GpsL1CaPcpsCUDAAcquisitionTest : public ::testing::Test {
protected:
    GpsL1CaPcpsCUDAAcquisitionTest() {
    ...
    std::shared_ptr <GpsL1CaPcpsCUDAAcquisition> acquisition;
...
void GpsL1CaPcpsCUDAAcquisitionTest::init() {
    message = 0;
...
void GpsL1CaPcpsCUDAAcquisitionTest::config_1() {
    ...
    config->set_property("Acquisition_1C.implementation", "GPS_L1_CA_PCPS_CUDA_Acquisition");
...
void GpsL1CaPcpsCUDAAcquisitionTest::config_2() {
    gnss_synchro.Channel_ID = 0;
    gnss_synchro.System = 'G';
    std::string signal = "1C";
    signal.copy(gnss_synchro.Signal, 2, 0);
    ...
    num_of_realizations = 100;  // Change here the number of realizations
...
void GpsL1CaPcpsCUDAAcquisitionTest::start_queue() {
    stop = false;
    ch_thread = boost::thread(&GpsL1CaPcpsCUDAAcquisitionTest::wait_message, this);
}


void GpsL1CaPcpsCUDAAcquisitionTest::wait_message() {
...
void GpsL1CaPcpsCUDAAcquisitionTest::process_message() {
    if (message == 1) {
        detection_counter++;

        // The term -5 is here to correct the additional delay introduced by the FIR filter
        double delay_error_chips = std::abs(static_cast<double>(expected_delay_chips) -
                                            static_cast<double>(gnss_synchro.Acq_delay_samples - 5) * 1023.0 /
    ...
    std::cout << "Progress: "
              << round(static_cast<float>(realization_counter) / static_cast<float>(num_of_realizations) * 100)
              << "% \r" << std::flush;
    ...
    if (realization_counter == num_of_realizations) {
        ...
        Pfa_p = static_cast<double>(detection_counter - correct_estimation_counter) /
                static_cast<double>(num_of_realizations);
...
void GpsL1CaPcpsCUDAAcquisitionTest::stop_queue() {
    stop = true;
}


TEST_F(GpsL1CaPcpsCUDAAcquisitionTest, Instantiate)
{
    config_1();
    acquisition = std::make_shared<GpsL1CaPcpsCUDAAcquisition>(config.get(), "Acquisition_1C", 1, 1);
}


TEST_F(GpsL1CaPcpsCUDAAcquisitionTest, ConnectAndRun)
{
    ...
    acquisition = std::make_shared<GpsL1CaPcpsCUDAAcquisition>(config.get(), "Acquisition_1C", 1, 1);
    boost::shared_ptr <GpsL1CaPcpsCUDAAcquisitionTest_msg_rx> msg_rx = GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_make(channel_internal_queue);
...
TEST_F(GpsL1CaPcpsCUDAAcquisitionTest, ValidationOfResults)
{
    config_1();

    acquisition = std::make_shared<GpsL1CaPcpsCUDAAcquisition>(config.get(), "Acquisition", 1, 1);
    boost::shared_ptr <GpsL1CaPcpsCUDAAcquisitionTest_msg_rx> msg_rx = GpsL1CaPcpsCUDAAcquisitionTest_msg_rx_make(channel_internal_queue);
...
TEST_F(GpsL1CaPcpsCUDAAcquisitionTest, ValidationOfResultsProbabilities)
{
    config_2();
