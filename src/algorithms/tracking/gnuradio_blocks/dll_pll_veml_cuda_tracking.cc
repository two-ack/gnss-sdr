/*!
 * \file dll_pll_veml_cuda_tracking.cc
...
#include "dll_pll_veml_cuda_tracking.h"
...
dll_pll_veml_cuda_tracking_sptr dll_pll_veml_cuda_make_tracking(dllpllconf_cuda_t conf_)
{
    return dll_pll_veml_cuda_tracking_sptr(new dll_pll_veml_cuda_tracking(conf_));
}


void dll_pll_veml_cuda_tracking::forecast(int noutput_items,
...
dll_pll_veml_cuda_tracking::dll_pll_veml_cuda_tracking(dllpllconf_cuda_t conf_) : gr::block("dll_pll_veml_cuda_tracking", gr::io_signature::make(1, 1, sizeof(gr_complex)),
                                                                             gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)))
{
    trk_parameters = conf_;
    cu_selected_device = init_cuda();
...
//    gpuErrchk(cudaMalloc((void**)&cu_in, sizeof(cuComplex) * 2 * trk_parameters.vector_length));
    ...
    multicorrelator_cuda.init(cu_selected_device, 2 * trk_parameters.vector_length, d_code_length_chips, d_n_correlator_taps);
...
void dll_pll_veml_cuda_tracking::start_tracking()
{
...
dll_pll_veml_cuda_tracking::~dll_pll_veml_cuda_tracking()
{
...
bool dll_pll_veml_cuda_tracking::acquire_secondary()
{
...
bool dll_pll_veml_cuda_tracking::cn0_and_tracking_lock_status()
{
...
void dll_pll_veml_cuda_tracking::do_correlation_step(const gr_complex *input_samples)
{
    ...
    multicorrelator_cuda.set_input_output_vectors(d_correlator_outs, input_samples);
    multicorrelator_cuda.Carrier_wipeoff_multicorrelator_resampler(
    ...
    if (trk_parameters.track_pilot)
    {
        correlator_data_cuda.set_input_output_vectors(d_Prompt_Data, input_samples);
        correlator_data_cuda.Carrier_wipeoff_multicorrelator_resampler(
                d_rem_carr_phase_rad,
                d_carrier_phase_step_rad,
                static_cast<float>(d_rem_code_phase_chips) * static_cast<float>(d_code_samples_per_chip),
                static_cast<float>(d_code_phase_step_chips) * static_cast<float>(d_code_samples_per_chip),
...
void dll_pll_veml_cuda_tracking::run_dll_pll()
{
    // ################## PLL ##########################################################
    // PLL discriminator
    if (d_cloop)
...
void dll_pll_veml_cuda_tracking::clear_tracking_vars()
{
    std::fill_n(d_correlator_outs, d_n_correlator_taps, gr_complex(0.0, 0.0));
...
void dll_pll_veml_cuda_tracking::update_tracking_vars()
{
...
void dll_pll_veml_cuda_tracking::save_correlation_results()
{
    if (d_secondary)
    {
...
void dll_pll_veml_cuda_tracking::log_data(bool integrating)
{
    if (trk_parameters.dump)
    {
...
int dll_pll_veml_cuda_tracking::save_matfile()
{
    // READ DUMP FILE
...
void dll_pll_veml_cuda_tracking::set_channel(unsigned int channel)
{
    gr::thread::scoped_lock l(d_setlock);
    d_channel = channel;
...
void dll_pll_veml_cuda_tracking::set_gnss_synchro(Gnss_Synchro *p_gnss_synchro)
{
    gr::thread::scoped_lock l(d_setlock);
...
int dll_pll_veml_cuda_tracking::general_work(int noutput_items __attribute__((unused)), gr_vector_int &ninput_items,
                                        gr_vector_const_void_star &input_items, gr_vector_void_star &output_items)
{
    gr::thread::scoped_lock l(d_setlock);
    const gr_complex *in = reinterpret_cast<const gr_complex *>(input_items[0]);
//    gpuErrchk(cudaMemcpy(cu_in, in, sizeof(cuComplex) * 2 * trk_parameters.vector_length, cudaMemcpyHostToDevice));
