/*!
 * \file gps_l1_ca_dll_pll_tracking_cuda.cc
...
GpsL1CaDllPllTrackingCUDA::GpsL1CaDllPllTrackingCUDA(
        ConfigurationInterface* configuration, std::string role,
        unsigned int in_streams, unsigned int out_streams) : role_(role), in_streams_(in_streams), out_streams_(out_streams)
{
    dllpllconf_cuda_t trk_param;
    DLOG(INFO) << "role " << role;
    ...
    if (item_type.compare("gr_complex") == 0)
    {
        ...
        tracking_ = dll_pll_veml_cuda_make_tracking(trk_param);
    }
    else
    {
        item_size_ = sizeof(gr_complex);
...
void GpsL1CaDllPllTrackingCUDA::start_tracking()
{
 ...
 */
void GpsL1CaDllPllTrackingCUDA::set_channel(unsigned int channel)
{
    channel_ = channel;
...
void GpsL1CaDllPllTrackingCUDA::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
    tracking_->set_gnss_synchro(p_gnss_synchro);
}


void GpsL1CaDllPllTrackingCUDA::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void GpsL1CaDllPllTrackingCUDA::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
    { /* top_block is not null */
    };
...
gr::basic_block_sptr GpsL1CaDllPllTrackingCUDA::get_left_block()
{
    return tracking_;
}


gr::basic_block_sptr GpsL1CaDllPllTrackingCUDA::get_right_block()
{
    return tracking_;
}
