...
static inline void volk_gnsssdr_32f_resamplerxnpuppet_32f_neon64(float* result, const float* local_code, unsigned int num_points)
{
    int code_length_chips = 2046;
    float code_phase_step_chips = ((float)(code_length_chips) + 0.1) / ((float)num_points);
    int num_out_vectors = 3;
    ...
    volk_gnsssdr_32f_xn_resampler_32f_xn_neon64(result_aux, local_code, rem_code_phase_chips, code_phase_step_chips, shifts_chips, code_length_chips, num_out_vectors, num_points);
