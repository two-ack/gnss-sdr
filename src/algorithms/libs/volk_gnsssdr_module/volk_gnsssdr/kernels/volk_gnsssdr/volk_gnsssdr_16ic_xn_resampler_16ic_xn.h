...
#include <arm_neon.h>
static inline void volk_gnsssdr_16ic_xn_resampler_16ic_xn_neon64(lv_16sc_t** result, const lv_16sc_t* local_code, float rem_code_phase_chips, float code_phase_step_chips, float* shifts_chips, unsigned int code_length_chips, int num_out_vectors, unsigned int num_points)
{
    lv_16sc_t** _result = result;
    const unsigned int neon_iters = num_points / 4;
    const int32x4_t ones = vdupq_n_s32(1);
